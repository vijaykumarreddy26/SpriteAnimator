 ...
 // function to calculating top and left of the image to display
  ...
  // function to stop animation 
   _this.stop=function(flag){
   	if(_this.intervalId){
   			(_this.options.requestAnimation) ? window.cancelAnimationFrame(_this.intervalId)  : window.clearTimeout(_this.intervalId);
       _this.tile=0;
         if( ! flag){
      _this.presentSequnce=null;
	  _this.allowContinuousPlay=false;
   ...
   //function to create animating element it will create canvas or div depend on canvasMode
    ...
    _this.createChild=function(element){
		...
		_this.$el=(_this.options.canvasMode)?$("<canvas style='position:relative;top:0px;left:0px' class="+_this.options.preffedClassName+" width="+_this.width+" height="+_this.width+"></canvas>"): $("<div style='position:relative;top:0px;left:0px;width:"+_this.width+"px;height:"+_this.height+"px;background-image:url("+_this.options.currentAnimation.options.imagePath+"); background-position: 0px 0px;' class="+_this.options.preffedClassName+"></div>");
	...
	_this.start=function(customsequence){
		...
		_this.temperarySequence=customsequence;
			...
			for(var i=0;i<_this.options.currentAnimation.options.frameSequence.length;i++)
		    _this.temperarySequence[i]=_this.options.currentAnimation.options.frameSequence[i];
		}
		...
		if(_this.rev){
			_this.temperarySequence=_this.temperarySequence.reverse();
		}
	...
	_this.playBacktoBackAnimation=function(animArray,rev){
	...
	// function to move to particular frame in sequence and play
	/*_this.goToAndPlay=function(frameNumber){
		if(_this.goTo(frameNumber)){
